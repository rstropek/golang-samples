...
bool hasQueen(const bitset<SIZE>& board, size_t x, size_t y) {
...
void Print(const bitset<SIZE> & board) {
...
int main() {
  ...
  std::chrono::duration<float> dfs = t2 - t1;
  ...
  cout << "Took " << dfs.count() << " seconds" << endl;
  return 0;
}
